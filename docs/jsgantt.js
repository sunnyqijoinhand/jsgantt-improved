		...
		2: [function (require, module, exports) {
				...
				exports.GanttChart = function (pDiv, pFormat, pGanttHeight) {
					...
					this.Draw = function () {
						...
						$('.gantt').attr('unselectable', 'on').css('user-select', 'none').on('selectstart', false);
						var vGanttContainer = $('#' + this.vDivId + '>div.gchartcontainer');
						var vGanttLeftPart = $('#' + this.vDivId + '>div.gchartcontainer>div.gmainleft');
						var vGanttRightPart = $('#' + this.vDivId + '>div.gchartcontainer>div.gmainright');
						...
						if ($(vGanttListBodyId).height() > this.vGanttHeight - $(vGanttListHeadId).outerHeight(true) || $(vGanttChartBodyId).height() > this.vGanttHeight - $(vGanttChartHeadId).outerHeight(true)) {
							...
							var vScrollControllerId = this.vDivId + '_OSgwn_GANTT_SCROLL_CONTROLLER';
							/*
							vGanttRightPart.width(vGanttRightPart.width() - 30); //resize right part
							var vScrollControllerDiv = $('<div style="width:30px;display:inline-block;"><div style="height:' + $(vGanttChartHeadId).height() + 'px;"></div><div id="' + vScrollControllerId + '" class="gantt-scroll-controll" style="height:' + (this.vGanttHeight - $(vGanttChartHeadId).height()) + 'px;overflow-x:hidden;overflow-y:auto;"><div id="' + vScrollControllerId + '_BODY"></div></div></div>');
							vGanttContainer.append(vScrollControllerDiv);
							setInterval(function () {
								$('#' + vScrollControllerId + '_BODY').height($(vGanttChartTableId).outerHeight(true));
							}, 50);
							*/
